...
export interface DialxOpenAIProvider extends ProviderV3 {
   ...
   */
  tools: typeof dialxOpenaiTools;
}

export interface DialxOpenAIProviderSettings {
  /**
  ...
  /**
   * Use useResponsesModels for specific model types. Set to true to use responses language model.
   */
  useResponsesModel?: boolean;
}

/**
 * Create a Dialx OpenAI provider instance.
 */
export function createDialx(
  options: DialxOpenAIProviderSettings = {},
): DialxOpenAIProvider {
  const getHeaders = () => {
    const baseHeaders = {
      'api-key': loadApiKey({
        ...
        description: 'Dialx OpenAI',
      }),
      ...options.headers,
    };
    return withUserAgentSuffix(baseHeaders, `ai-sdk/dialx/${VERSION}`);
  };
  ...
  const getResourceName = () =>
    loadSetting({
      ...
      environmentVariableName: 'AZURE_RESOURCE_NAME',
        description: 'Dialx OpenAI resource name',
    });
  ...
  provider.tools = dialxOpenaiTools;
  return provider;
}

/**
 * Default Dialx OpenAI provider instance.
 */
export const dialx = createDialx();
